...
export default class extends Controller {
  ...
  async #setCurrentFrom(element) {
    const selectedItem = this.#visibleItems.find((item) =>
  ...
  get #parentNavigableListController() {
    const parentNavigableList = this.element.parentElement?.closest(
  ...
  async #selectCurrentElementInParent() {
    ...
    if (parentController) {
      const parentItem = this.element.closest(
  ...
  #nestedNavigableListController() {
    const nestedElement = this.currentItem?.querySelector(
      '[data-controller~="navigable-list"]',
    );
    if (nestedElement) {
      return this.application.getControllerForElementAndIdentifier(
  ...
  #activateManualSelection() {
    const preselectedItem = this.itemTargets.find((item) =>
      item.hasAttribute(this.selectionAttributeValue),
    );
    if (preselectedItem) {
  ...
  #relayNavigationToParentNavigableList(event) {
    ...
    if (parentController) {
      parentController.element.focus({
        preventScroll: !parentController.autoScrollValue,
      });
  ...
  #clickCurrentItem(event) {
    if (
      this.actionableItemsValue &&
      this.currentItem &&
      this.#visibleItems.length &&
      this.#isFocusContainedOnNavigableItem
    ) {
      const clickableElement =
        this.currentItem.querySelector("a,button") || this.currentItem;
  ...
  get #isFocusContainedOnNavigableItem() {
    return (
      !this.onlyActOnFocusedItemsValue ||
      this.itemTargets.some(
        (item) =>
    ...
    return {
      ...
      ArrowRight: (event) => {
        if (this.supportsHorizontalNavigationValue) {
          this.#handleArrowKey(event, this.#selectNext);
        }
      },
      ArrowLeft: (event) => {
        if (this.supportsHorizontalNavigationValue) {
          this.#handleArrowKey(event, this.#selectPrevious);
        }
      },
      Enter: (event) => {
        if (event.shiftKey) {
